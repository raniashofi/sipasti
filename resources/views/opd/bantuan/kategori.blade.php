...
<main class="flex-1 w-full max-w-screen-lg mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-10">
    ...
    {{-- Search --}}
    <form method="GET" action="{{ route('opd.bantuan.kategori', $kategori->id) }}" class="mb-6 sm:mb-7">
        <div class="flex gap-2 sm:gap-3 w-full sm:max-w-xl">
            <div class="flex-1 relative">
                <svg class="absolute left-3 sm:left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 shrink-0" fill="none"
                     stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 21l-4.35-4.35M17 11A6 6 0 105 11a6 6 0 0012 0z"/>
                </svg>
                <input type="text" name="search" value="{{ $search }}"
                       placeholder="Cari judul atau kata kunci..."
                       class="w-full pl-9 sm:pl-11 pr-3 sm:pr-4 py-2.5 rounded-xl text-sm bg-white border border-gray-200
                              focus:outline-none focus:ring-2 focus:ring-[#01458E]/20 focus:border-[#01458E]
                              placeholder-gray-300 transition-all">
            </div>
            <button type="submit"
                    class="shrink-0 px-4 sm:px-6 py-2.5 rounded-xl text-sm font-bold text-white transition hover:opacity-90 active:scale-95"
    ...
    {{-- Artikel List --}}
    @if($artikels->isEmpty())
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-6 sm:px-8 py-12 sm:py-14 text-center">
    ...
    <div class="space-y-2">
        ...
        <a href="{{ route('opd.bantuan.artikel', $artikel->id) }}"
           class="flex items-center justify-between bg-white rounded-xl border border-gray-100 shadow-sm px-4 sm:px-5 py-3.5 sm:py-4
                  hover:border-[#01458E]/30 hover:shadow-md transition-all group">
            <div class="flex-1 min-w-0 pr-3">
                <p class="text-sm font-semibold text-gray-800 group-hover:text-[#01458E] transition-colors line-clamp-2 sm:line-clamp-1">
            ...
            <div class="flex items-center gap-2 sm:gap-3 shrink-0">
                ...
                <span class="text-[11px] text-gray-400 hidden sm:block whitespace-nowrap">{{ number_format($artikel->total_views) }} views</span>
                @endif
                <svg class="w-4 h-4 text-gray-300 group-hover:text-[#01458E] transition-colors shrink-0" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/>
                </svg>
            </div>
        </a>
    ...
    {{-- CTA --}}
    <div class="mt-8 sm:mt-10 rounded-2xl border border-gray-200 bg-white shadow-sm px-5 sm:px-7 py-5 sm:py-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
        ...
        <a href="{{ route('opd.diagnosis.index') }}"
           class="shrink-0 w-full sm:w-auto text-center px-5 py-2.5 rounded-xl text-sm font-bold text-white transition hover:-translate-y-0.5 hover:shadow-lg active:scale-95"
           style="background:#01458E;">
            Buat Pengaduan
        </a>
    </div>
