...
<head>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .prose-artikel table { width: 100%; border-collapse: collapse; margin: 0.85rem 0; font-size: 0.825rem; }
        .prose-artikel th { background: #F1F5F9; font-weight: 600; text-align: left; padding: 0.5rem 0.75rem; border: 1px solid #E5E7EB; }
...
{{-- ══ Hero / Header Image (paling atas, full-width) ══ --}}
@if($artikel->header_image)
<div class="w-full bg-gray-900 overflow-hidden" style="max-height:360px;">
...
<main class="flex-1 w-full max-w-screen-xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-10">

    {{-- Breadcrumb --}}
    <div class="flex items-center gap-2 text-xs text-gray-400 mb-5 sm:mb-7 flex-wrap">
        ...
        <span class="text-gray-600 font-medium truncate max-w-[14rem] sm:max-w-xs">{{ $artikel->nama_artikel_sop }}</span>
    </div>

    <div class="grid grid-cols-1 lg:grid-cols-4 gap-5 sm:gap-6 items-start">
        ...
        <div class="lg:col-span-3 min-w-0">
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                ...
                {{-- Header artikel --}}
                <div class="px-5 sm:px-8 py-5 sm:py-7 border-b border-gray-100">
                    @if($artikel->kategori)
                    ...
                    @endif
                    <h1 class="text-lg sm:text-xl font-bold text-gray-900 leading-snug">{{ $artikel->nama_artikel_sop }}</h1>
                    @if($artikel->deskripsi_singkat)
                    <p class="text-sm text-gray-500 mt-2 leading-relaxed">{{ $artikel->deskripsi_singkat }}</p>
                    @endif
                    <div class="flex flex-wrap items-center gap-x-4 gap-y-1.5 mt-4 text-xs text-gray-400">
                ...
                {{-- Isi Konten --}}
                <div class="px-5 sm:px-8 py-5 sm:py-7 overflow-x-auto">
                ...
                <div class="px-5 sm:px-8 pb-5 sm:pb-7">
                    <div class="flex items-center gap-3 p-4 rounded-xl border border-gray-200 bg-gray-50">
                        <div class="w-9 h-9 rounded-lg flex items-center justify-center shrink-0"
                            ...
                            <svg style="color:#01458E;width:18px;height:18px;" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                ...
                <div class="px-5 sm:px-8 py-5 sm:py-6 border-t border-gray-100"
                     x-data="{
                        rating: {{ $myRating ?? 0 }},
                        hover: 0,
            ...
            <div class="mt-5 sm:mt-6">
                ...
                <div class="space-y-2">
                    ...
                    <a href="{{ route('opd.bantuan.artikel', $rel->id) }}"
                       class="flex items-center justify-between bg-white rounded-xl border border-gray-100 shadow-sm px-4 sm:px-5 py-3.5
                              hover:border-[#01458E]/30 hover:shadow-md transition-all group">
                        <span class="text-sm text-gray-700 group-hover:text-[#01458E] transition-colors line-clamp-2 sm:line-clamp-1 pr-3">
                            {{ $rel->nama_artikel_sop }}
                        </span>
                        <svg class="w-4 h-4 text-gray-300 group-hover:text-[#01458E] shrink-0 transition-colors" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 4.5l7.5 7.5-7.5 7.5"/>
                        </svg>
                    </a>
                    @endforeach
        ...
        <div class="lg:col-span-1 space-y-4 lg:sticky lg:top-24">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-5"
                 ...
                 ">
                {{-- Header daftar isi dengan toggle di mobile --}}
                <button type="button"
                        class="w-full flex items-center justify-between lg:cursor-default"
                        @click="open = !open">
                    <p class="text-[11px] font-bold text-gray-400 uppercase tracking-widest">Daftar Isi</p>
                    <svg class="w-4 h-4 text-gray-400 transition-transform lg:hidden"
                         :class="open ? 'rotate-180' : ''"
                         fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 8.25l-7.5 7.5-7.5-7.5"/>
                    </svg>
                </button>
                <nav class="toc-scroll max-h-72 overflow-y-auto mt-3 space-y-0.5"
...
<footer class="text-center py-5 sm:py-6 mt-auto border-t border-gray-200 bg-white text-gray-400 text-xs font-medium">
