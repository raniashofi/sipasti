...
{{-- Page header — di luar container biru --}}
    <div class="mb-8">
        ...
        <a href="{{ route('opd.diagnosis.index') }}"
           class="inline-flex items-center gap-1.5 text-sm font-semibold text-gray-500 hover:text-[#01458E] transition-colors mb-4">
        ...
        <h1 class="text-xl font-bold text-gray-900 mb-1">Kategori: {{ $kategoriNama }}</h1>
    ...
    <div class="px-24 pt-20 pb-12" style="background-color:#E8EEF5;">
        ...
        {{-- Question label + text --}}
        <div class="mb-6 pb-6 border-b border-[#C8D5E8]">
            <p class="text-sm text-gray-400 font-normal mb-1">Question {{ $qNum }}</p>
        ...
        <div class="flex items-center justify-center gap-3 mt-24">

            @if($qNum > 1)
            <button type="button"
                    onclick="history.back()"
                    class="flex items-center gap-2 px-7 py-3 rounded-full border-2 border-[#01458E]
                           text-sm font-semibold text-[#01458E] bg-white hover:bg-[#EEF3F9] transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18"/>
                </svg>
                Kembali
